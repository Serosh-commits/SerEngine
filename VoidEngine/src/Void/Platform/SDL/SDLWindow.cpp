...
#include "Void/Events/KeyEvent.h"
...
namespace Void {
    ...
    void SDLWindow::Init(const WindowProps& props) {
        ...
        VOID_INFO("Creating window {0} ({1}, {2})", props.Title, props.Width, props.Height);

        if (SDL_Init(SDL_INIT_VIDEO) < 0) {
            VOID_ERROR("Could not initialize SDL! SDL_Error: {0}", SDL_GetError());
            return;
        }
        ...
        if (!m_Window) {
            VOID_ERROR("Could not create window! SDL_Error: {0}", SDL_GetError());
            return;
        }
